...
function sumOfCodes(str) {
  if (str && str.length > 0) {
    const arr = Array.from(str).map((item, index) => str.charCodeAt(index));
...
function formatTime(minutes, seconds) {
  const str1 = String(minutes).padStart(2, '0');
  const str2 = String(seconds).padStart(2, '0');
  return `${str1}:${str2}`;
}
...
function countVowels(str) {
  ...
  const arr = Array.from(str);
  arr.filter((letter) => vovels.includes(letter));
  return arr.filter((letter) => vovels.includes(letter)).length;
}
...
function isPalindrome(str) {
  ...
  const strReverse = Array.from(lower).reverse().join('');
  return lower === strReverse;
}
...
function findLongestWord(sentence) {
  ...
  const longestIndex = arr.reduce(
    (acc, item, index) => (arr[acc].length < item.length ? index : acc),
    0
  );
...
function reverseWords(str) {
  const arr = str.split(' ');
  const rev = arr.map((item) => Array.from(item).reverse().join(''));
  return rev.join(' ');
}
...
function encodeToRot13(str) {
  ...
  const newArr = arr.map((item) => {
    const code = item.charCodeAt(item);

    let res = '';
    if (code < 65 || code > 122) {
      res = String.fromCharCode(code);
    } else if ((code >= 65 && code < 78) || (code >= 97 && code < 110)) {
      res = String.fromCharCode(code + 13);
    } else if ((code >= 78 && code < 91) || (code >= 110 && code < 123)) {
      res = String.fromCharCode(code - 13);
    }
...
function getCardId(value) {
  ...
  const second = '♣♦♥♠';
  const char1 = value.slice(0, -1);
  const char2 = value.slice(-1);
  const indexChar1 = first.indexOf(char1);
  const indexChar2 = second.indexOf(char2);
  return indexChar1 + indexChar2 * 13;
}
